...
// 54-001 (ERROR)
// Cap I: Rînd 10 >= (Rînd 11 + Rînd 14 + Rînd 15 + Rînd 16 + Rînd 17 + Rînd 18 + Rînd 19) pe toate coloanele
function validateRow10Sum(values) {
    let errors = [];

    // Cap.I are coloanele 1..8
    let maxColumns = 8;
    ...
    for (let i = 1; i <= maxColumns; i++) {
        ...
        // Expected minimum
        let expectedMin = row11 + row14 + row15 + row16 + row17 + row18 + row19;

        // Check: Row 10 must be >= expectedMin
        if (row10 < expectedMin) {
            errors.push({
                fieldName: `CAP1_R10_C${i}`,
                msg: Drupal.t(
                    `Cod eroare: 54-001, Cap I, Rând 10 trebuie să fie ≥ suma rândurilor 11, 14, 15, 16, 17, 18 și 19 în coloana ${i}. ` +
                    `Valoare actuală: ${row10}, Valoare minimă așteptată: ${expectedMin}`
                )
            });
        }
    }
